...
int main(int argc, char **argv) {
  ...
  // 等待 TF 坐标系准备好
  RCLCPP_INFO(rclcpp::get_logger("demo4"), "Waiting for cube TF frames to be available...");
  ...
  for (size_t i = 0; i < to_frame_list.size(); i++) {
    ...
    while (retry_count < max_retries) {
      ...
      retry_count++;
      RCLCPP_WARN(rclcpp::get_logger("demo4"), "Waiting for %s TF (attempt %d/%d)...",
                  to_frame_list[i].c_str(), retry_count, max_retries);
      rclcpp::sleep_for(std::chrono::seconds(1));
    }

    if (cube_pose.empty()) {
      RCLCPP_WARN(rclcpp::get_logger("demo4"), "Failed to get pose for %s after %d attempts, skipping",
                  to_frame_list[i].c_str(), max_retries);
      continue;
    }
